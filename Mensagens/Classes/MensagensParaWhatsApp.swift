...
//
//  Created by Lucas Barcelos on 21/02/18.
...
public class MensagensParaWhatsApp: NSObject {
    ...
    static func definirPeriodo() -> Periodo? {
        ...
        guard let meioDia = Calendar.current.date(bySettingHour: 12, minute: 0, second: 0, of: agora) else {
            return nil
        }
        
        guard let seisHoras = Calendar.current.date(bySettingHour: 18, minute: 0, second: 0, of: agora) else {
            return nil
        }
    ...
    public static func mensagemRandom() -> String? {
        ...
        guard let periodoAtual = MensagensParaWhatsApp.definirPeriodo() else {
            return nil
        }
        
        // Count das mensagens do período atual
        guard let countMensagens = mensagens[periodoAtual]?.count else {
            return nil
        }
        
        // Criação de número aleatório com base na quantidade de mensagens do período
        let numeroAleatorio = Int(arc4random_uniform(UInt32(countMensagens)))
        
        // Retornar uma mensagem aleatória
